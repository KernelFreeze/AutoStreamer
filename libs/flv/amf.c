...
static amf_data *amf_associative_array_read(amf_read_proc read_proc, void *user_data) {
    ...
    while (1) {
        ...
        if (amf_string_get_size(name) == 0 || error_code == AMF_ERROR_END_TAG ||
            error_code == AMF_ERROR_UNKNOWN_TYPE) {
            /* end tag or unknown element: end of data, exit loop */
            amf_data_free(name);
            amf_data_free(element);
            break;
...
size_t amf_data_size(const amf_data *data) {
    ...
    if (data != NULL) {
        ...
        case AMF_TYPE_ASSOCIATIVE_ARRAY:
            ...
            while (node != NULL) {
                s += sizeof(uint16) +
                     (size_t)amf_string_get_size(amf_associative_array_get_name(node));
...
static size_t amf_associative_array_write(
    const amf_data *data, amf_write_proc write_proc, void *user_data) {
    amf_node *node;
    size_t w = 0;
    uint32_be s;
    uint16_be filler = swap_uint16(0);
        ...
        case AMF_TYPE_STRING:
            if (data->string_data.mbstr != NULL) {
                return amf_string_new(
                    (byte *)strdup((char *)amf_string_get_bytes(data)), amf_string_get_size(data));
            } else {
                return amf_str(NULL);
            }
        case AMF_TYPE_NULL:
...
amf_data *amf_object_get(const amf_data *data, const char *name) {
    if (data != NULL) {
        ...
        while (node != NULL) {
            if (strncmp((char *)(node->data->string_data.mbstr), name,
                    (size_t)(node->data->string_data.size)) == 0) {
                node = node->next;
